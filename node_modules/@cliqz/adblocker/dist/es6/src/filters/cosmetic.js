...
export default class CosmeticFilter {
    ...
    static parse(line, debug = false) {
        ...
        if (line.endsWith(':remove()')) {
            // ##selector:remove()
            mask = setBit(mask, 64 /* remove */);
            mask = setBit(mask, 128 /* extended */);
            line = line.slice(0, -9);
        }
        ...
        if (line.charCodeAt(suffixStartIndex) === 94 /* '^' */) {
            ...
            if (getBit(mask, 1 /* unhide */) === false && selector.length === 0) {
                return null;
            }
        }
        else {
        ...
        if (domains === undefined && getBit(mask, 128 /* extended */) === true) {
            return null;
        }
        if (selector !== undefined) {
            ...
            if (getBit(mask, 2 /* scriptInject */) === false &&
                getBit(mask, 64 /* remove */) === false &&
                getBit(mask, 128 /* extended */) === false &&
    ...
    static deserialize(buffer) {
        ...
        const isUnicode = getBit(mask, 4 /* isUnicode */);
    ...
    isExtended() {
        return getBit(this.mask, 128 /* extended */);
    }
    isRemove() {
        return getBit(this.mask, 64 /* remove */);
    }
    isUnhide() {
